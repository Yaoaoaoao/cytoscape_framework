  ...
  CanvasRenderer.prototype.drawText = function(context, element, textX, textY) {
    ...
    if ( text != undefined && !isNaN(textX) && !isNaN(textY) ) {
      ...
      if (lineWidth > 0) {
		...
		X = textX + 3;
		Y = textY - 0.5*12*(textLine-1);
		for (var i in textArr){
			context.strokeText("" + textArr[i], X, Y);
			Y += 12;
		}
      }
...
;(function($$){ 'use strict';
  ...
  CanvasRenderer.prototype.png = function( options ){
 		var canvas = this.bufferCanvasImage( options );
 		canvas.toBlob(function(blob){
 		var networkFileName = $("#networkFileName").val();
 		saveAs(blob, networkFileName+'.png');
 	});
